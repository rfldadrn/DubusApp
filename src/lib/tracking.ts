...
export async function trackOrderByCode(code: string): Promise<TrackingResult> {
  if (!code?.trim()) {
  ...
  if (
    transaction.statusTransaction.code === "SELESAI" &&
      ...
      items: transaction.items.map((i) => {
        ...
        for (const log of itemLogs) {
          if (!seenStatuses.has(log.toStatusId)) {
            ...
            const status = statusMap.get(log.toStatusId);
            if (status) {
              completedSteps.push({
